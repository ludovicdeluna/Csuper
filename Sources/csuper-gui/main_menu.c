...
G_MODULE_EXPORT void openAssistantNewCsu(GtkWidget *widget, gpointer data)
{
    ...
    g_signal_connect(user_data->ptr_new_csu_file_assistant,"delete-event", G_CALLBACK(gtk_widget_destroy),user_data);
    g_signal_connect(user_data->ptr_new_csu_file_assistant,"cancel", G_CALLBACK(gtk_widget_destroy),user_data);
    ...
    gtk_assistant_set_page_type(GTK_ASSISTANT(user_data->ptr_new_csu_file_assistant),grid_2,GTK_ASSISTANT_PAGE_CONFIRM);
...
G_MODULE_EXPORT void validAssistantNewCsuOne(GtkWidget *widget, gpointer data)
{
    ...
    GtkWidget *entry = GTK_WIDGET(gtk_builder_get_object(user_data->ptr_builder,"grid_new_csu_file_assistant_1_entry_name"));
    if (!entry)
